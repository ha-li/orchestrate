package worker

import (
	"log/slog"

	"github.com/c9s/goprocinfo/linux"
)

type (

	// these types give access to the data in /proc of linux os
	Stats struct {
		// we will provide metrics on
		// - mem
		// - disk
		// - cpu
		// - load
		MemStats  *linux.MemInfo
		DiskStats *linux.Disk
		CpuStats  *linux.CPUStat
		LoadStats *linux.LoadAvg
		TaskCount int
	}
)

// please keep the same family of metrics together for easy reading

// mem metrics

func (s *Stats) MemTotalKb() uint64 {
	return s.MemStats.MemTotal
}

func (s *Stats) MemAvailableKb() uint64 {
	return s.MemStats.MemAvailable
}

func (s *Stats) MemUsedKb() uint64 {
	return s.MemStats.MemTotal - s.MemStats.MemAvailable
}

func (s *Stats) MemUsedPercent() uint64 {
	return s.MemStats.MemAvailable / s.MemStats.MemTotal
}

// disk metrics

func (s *Stats) DiskTotal() uint64 {
	return s.DiskStats.All
}

func (s *Stats) DiskFree() uint64 {
	return s.DiskStats.Free
}

func (s *Stats) DiskUsed() uint64 {
	return s.DiskStats.Used
}

func (s *Stats) CpuUsage() float64 {
	idle := s.CpuStats.Idle + s.CpuStats.IOWait
	nonIdle := s.CpuStats.User + s.CpuStats.Nice + s.CpuStats.System + s.CpuStats.IRQ + s.CpuStats.SoftIRQ + s.CpuStats.Steal
	total := idle + nonIdle

	if total == 0 {
		return 0.00
	}
	return (float64(total) - float64(idle)) / float64(total)
}

func GetStats() *Stats {
	// these all build on /proc
	return &Stats{
		MemStats:  GetMemoryInfo(),
		DiskStats: GetDiskInfo(),
		CpuStats:  GetCpuStats(),
		LoadStats: GetLoadAvg(),
	}
}

func GetMemoryInfo() *linux.MemInfo {
	memstats, err := linux.ReadMemInfo("/proc/meminfo")
	if err != nil {
		slog.Error("Error reading from /proc/meminfo")
		return &linux.MemInfo{}
	}
	return memstats
}

func GetDiskInfo() *linux.Disk {
	diskstats, err := linux.ReadDisk("/")
	if err != nil {
		slog.Error("Error reading from /")
		return &linux.Disk{}
	}
	return diskstats
}

func GetCpuStats() *linux.CPUStat {
	stats, err := linux.ReadStat("/proc/stat")
	if err != nil {
		slog.Error("Error reading from /proc/stat")
		return &linux.CPUStat{}
	}
	return &stats.CPUStatAll
}

func GetLoadAvg() *linux.LoadAvg {
	loadavg, error := linux.ReadLoadAvg("/proc/loadavg")
	if error != nil {
		slog.Error("Error reading from /proc/loadavg")
		return &linux.LoadAvg{}
	}
	return loadavg
}
